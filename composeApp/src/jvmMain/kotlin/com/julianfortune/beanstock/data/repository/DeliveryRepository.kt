...
class DeliveryRepository(private val database: Database) {
    ...
    fun getAllAsHeadlines(): Flow<List<DeliveryHeadline>> {
        return database.deliveryQueries.getAll().asFlow().mapToList(Dispatchers.IO).map { deliveries ->
            deliveries.map { d ->
    ...
    fun getDeliveryById(deliveryId: Long): Flow<Delivery?> {
        val deliveryFlow = database.deliveryQueries.getById(deliveryId).asFlow().mapToOneOrNull(Dispatchers.IO)

        val entriesFlow = database.deliveryEntryQueries.getByDeliveryId(deliveryId).asFlow().mapToList(Dispatchers.IO)
    ...
    private fun entryFromRow(row: GetByDeliveryId): Delivery.Entry {
        ...
        val program =
            row.programId?.let { id ->
                Program(
                    id,
                    row.programName ?: throw Exception("`programName` must be defined by foreign key constraints"),
                )
            }
        val account =
    ...
    private fun entryFromRow(row: GetAllByBasicReportCriteriaPaginated): Delivery.Entry =
        entryFromRow(
            GetByDeliveryId(
                row.id,
                row.deliveryId,
