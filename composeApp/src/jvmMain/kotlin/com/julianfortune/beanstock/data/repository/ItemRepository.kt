...
class ItemRepository(private val database: Database) : NamedEntityRepository<ItemHeadline> {
    ...
    override fun getAll(): Flow<List<ItemHeadline>> {
        return database.itemQueries.getAllItems().asFlow().mapToList(Dispatchers.IO).map { items ->
            items.map {
    ...
    fun getItemById(id: Long): Flow<Item> {
        return database.itemQueries.getItemWithCategoriesById(id).asFlow().mapToList(Dispatchers.IO).map { rows ->
            ...
            val savedWeights =
                first.savedWeightInCentigramsListJson?.let {
